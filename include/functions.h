...
Ml backpropagation(Ml ml, Vec input_vec, Vec output_vec) {
    ...
    for (int l = ml.size - 1; l > 0; --l) {
        ...
        for (unsigned int j = 0; j < ml.layers[l].neurons; ++j) {
            ...
            *TYPE_CAST_PTR(diff_activation, ml.data_type) = 2 * (TENSOR_INDEX(ml.layers[l].activation, j) - TENSOR_INDEX(gradient.layers[l].activation, j));
            ...
            *CAST_PTR(diff_sigmoid, float) = (sigmoid_func(current_z.data + j, temp_a, current_z.data_type), *CAST_PTR(temp_a, float)) * (1 - (sigmoid_func(current_z.data + j, temp_b, current_z.data_type), *CAST_PTR(temp_b, float)));
            ...
            for (unsigned int k = 0; k < ml.layers[l - 1].neurons; ++k) {
                TENSOR_INDEX(gradient.layers[l].weights, j * gradient.layers[l].weights.shape[0] + k) = (*TYPE_CAST_PTR(diff_activation, ml.data_type)) * (*TYPE_CAST_PTR(diff_sigmoid, ml.data_type)) * TENSOR_INDEX(ml.layers[l - 1].activation, k);
            }

            // Store the dC/db[j]^L
            TENSOR_INDEX(gradient.layers[l].biases, j) = (*TYPE_CAST_PTR(diff_activation, ml.data_type)) * (*TYPE_CAST_PTR(diff_sigmoid, ml.data_type));

            // Store the dC/da[j]^L
            TENSOR_INDEX(gradient.layers[l].activation, j) = (*TYPE_CAST_PTR(diff_activation, ml.data_type));
